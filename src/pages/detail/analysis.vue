...
import VMulChooser from '../../components/base/multiply-chooser'
...
export default {
  ...
  methods: {
    onParamChange (attr, val) {
      this[attr] = val                            //val为子组件传过来的数据，attr为相应的数据
      // console.log(attr,this[attr]) 
      this.getPrice()
    },
    getPrice () {
      let buyVersionsArray = _.map(this.versions, (item) => {      //多选形成的数组，需提取value并拼成字符串
        return item.value
      })
      let reqParams = {
  ...
  mounted () {                                              //组件渲染完，没选中选项前，可以有一个初始价格
    this.buyNum = 1
